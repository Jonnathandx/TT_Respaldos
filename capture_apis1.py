...
def main():
    ...
    client = ModbusTcpClient(host=MODBUS_IP_APIS1, port=MODBUS_PORT, timeout=3)
        ...
        while True:
            ...
            try:
                ...
                if response2.isError():
                    log.error(f"Error en lectura IFV2: {response2}")
                else:
                    insert_data(conn, response2.registers, "apis1_ifv2")

                if response3.isError():
                    log.error(f"Error en lectura IFV3: {response3}")
                else:
                    insert_data(conn, response3.registers, "apis1_ifv3")

                time.sleep(0.11)  # Intervalo de lectura (110ms)
